...
public class State {
        ...
        if (i < entries.size()) {
            ...
            if (!isRestrictionMatched()) {
                ...
                if (e instanceof TextElement) {
                    ...
                    getActionSet(entries, (i + 1), getActionSetList(), tails);
                } else {
                    processItem(real, i, tails);
    ...
    private final void processItem(final String value, int i,
        ...
        if (getTransitions().containsKey("0")) {
            ...
            //System.out.println("S vale " + state.getTemp());
            //if (tail.size() > 0) {
            if (!tails.containsKey(state.getTemp())) {
                tails.put(state.getTemp(), tail);
            } else {
                // si ya contiene, p.ej., S1
                ElementList existingTail = (ElementList) tails.get(state.getTemp());
                if (!tail.equals(existingTail)) {
                    // no seguimos procesando por este camino
                    // porque la cola asociada a otro S1 es diferente
                    // a la ya asignada
                    return;
                }
